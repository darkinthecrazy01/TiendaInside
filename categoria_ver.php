...
$DatosProductos = mysql_query($query_DatosProductos, $conexionInsite) or die(mysql_error());
$row_DatosProductos = mysql_fetch_assoc($DatosProductos);
$totalRows_DatosProductos = "0";
if (isset($_GET["cat"])) {
  $totalRows_DatosProductos = $_GET["cat"];
}
...
$varSubcategoria_DatosProductos = "0";
if (isset($_GET["sub"])) {
  $varSubcategoria_DatosProductos = $_GET["sub"];
}
mysql_select_db($database_conexionInsite, $conexionInsite);
$query_DatosProductos = sprintf("SELECT * FROM tblproducto WHERE tblproducto.intCategoria = %s AND tblproducto.strSubcategoria = %s", GetSQLValueString($varCategoria_DatosProductos, "int"),GetSQLValueString($varSubcategoria_DatosProductos, "text"));
$query_limit_DatosProductos = sprintf("%s LIMIT %d, %d", $query_DatosProductos, $startRow_DatosProductos, $maxRows_DatosProductos);
$DatosProductos = mysql_query($query_limit_DatosProductos, $conexionInsite) or die(mysql_error());
$row_DatosProductos = mysql_fetch_assoc($DatosProductos);
...
<div class="container">
  ...
  <div class="subcontenedor">
    ...
    <!-- end .sidebar1 --></div>
  <div class="content">
    <h1><!-- InstanceBeginEditable name="olantilla" --> Productos:
      <?php echo $row_DatosProductos['strSubcategoria']; ?><!-- InstanceEndEditable --></h1>
    <!-- InstanceBeginEditable name="Contenido" -->
    <div class= "resultadoproductos">
    <?php if ($totalRows_DatosProductos > 0) { // Show if recordset not empty ?>
        <?php do { ?>
        <div class="productos"><div class="fotoproducto"><img src="documentos/productos/<?php echo $row_DatosProductos['strImagen']; ?>" width="120" height="120" /></div>
        <div class="textoproducto">
          <a href="ver_producto.php?recordID=<?php echo $row_DatosProductos['idProducto']; ?>"> <?php echo $row_DatosProductos['strNombre']; ?></a>.<br />
          <br />
          Precio: $<?php echo $row_DatosProductos['dbPrecio']; ?> </div></div>
